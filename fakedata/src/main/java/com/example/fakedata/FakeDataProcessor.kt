...
class FakeDataProcessor(private val codeGenerator: CodeGenerator) : SymbolProcessor {
    ...
    private fun createInitializer(symbol: KSClassDeclaration): String {
        val properties = symbol.primaryConstructor?.parameters?.joinToString(",") { paramater ->
            val propertyName = paramater.name?.asString()
            val propertyType = paramater.type.resolve().declaration.qualifiedName!!.asString()
